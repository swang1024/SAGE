...
#   bash evaluation/run_openai.sh sage gpt-4o-mini smoke 8
#
# Usage:
...
bash "$EVAL/run_openai.sh" mem0 gpt-4o-mini full 8
bash "$EVAL/run_openai.sh" sage gpt-4o-mini full 8
...
#    The score table is both printed and saved to results_openai_full_bs8/scores_summary.txt.
SCORES_OUT="$REPO_ROOT/results_openai_full_bs8/scores_summary.txt"
"$ALT_PY" "$EVAL/generate_scores.py" results_openai_full_bs8 \
  --runs mem0_gpt-4o-mini/eval_metrics_judge4omini.json \
         sage_gpt-4o-mini/eval_metrics_judge4omini.json | tee "$SCORES_OUT"
