...
#     run_opensrc_full.sh reads OLLAMA_HOST and writes to results_full/.
bash evaluation/run_opensrc_full.sh mem0 "$OLLAMA_MODEL"
bash evaluation/run_opensrc_full.sh sage "$OLLAMA_MODEL"
...
#        The score table is both printed and saved to results_full/scores_summary.txt. ---
SCORES_OUT="results_full/scores_summary.txt"
if [ "${#METRIC_FILES[@]}" -gt 0 ]; then
  RUN_ARGS=()
  for f in "${METRIC_FILES[@]}"; do RUN_ARGS+=("$(basename "$f")"); done
  "$ALT_PY" evaluation/generate_scores.py results_full --runs "${RUN_ARGS[@]}" | tee "$SCORES_OUT"
fi
...
echo "Final score table saved to ${SCORES_OUT}"
