...
# The UPDATE_MERGE_PROMPT below is a deliberate choice, not a fixed default.
# In our experience the right length depends on the model doing the merge:
#   - Smaller / weaker models tend to do BETTER with the short prompt: a long,
#     rule-heavy prompt overloads them, and they either ignore the rules or
#     over-edit. The terse version keeps them focused and generally gives
#     better merge quality.
#   - Larger / more capable models tend to BENEFIT from the long prompt: they
#     can actually follow the explicit recall/precision rules (keep proper
#     nouns, additive multi-value facts, single-valued overwrites, etc.) and
#     produce more faithful merges.
# Pick the variant that matches the model you are running. The short version is
# active by default below; the long version is preserved (commented out) so you
# can swap it in for stronger models.

# --- Short prompt (default; recommended for smaller models) ---
UPDATE_MERGE_PROMPT = (
    "Existing memory: {existing}. New information: {new}. "
    "Write a concise, non-redundant merged memory preserving all distinct information. Max 2 sentences."
)

# --- Long prompt (recommended for larger, more capable models) ---
# UPDATE_MERGE_PROMPT = (
