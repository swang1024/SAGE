...
def test_thinking_tags_sync():
    ...
    # Sync novelty gate path uses only fact extraction LLM call.
    mock_llm.generate_response.return_value = '        <think>Sync fact extraction</think>  \n{"facts": ["User loves sci-fi"]}'
    ...
    assert len(result) == 1
    assert result[0]["memory"] == "User loves sci-fi"
