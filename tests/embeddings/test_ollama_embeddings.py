...
def test_embed_batch_texts(mock_ollama_client):
    config = BaseEmbedderConfig(model="nomic-embed-text", embedding_dims=512)
    embedder = OllamaEmbedding(config)
    ...
    texts = ["first", "second"]
    embeddings = embedder.embed_batch(texts, "add")

    mock_ollama_client.embed.assert_called_once_with(model="nomic-embed-text", input=texts)
    assert embeddings == [[0.1, 0.2], [0.3, 0.4]]


def test_embed_batch_mismatch_raises(mock_ollama_client):
    config = BaseEmbedderConfig(model="nomic-embed-text", embedding_dims=512)
    embedder = OllamaEmbedding(config)

    mock_ollama_client.embed.return_value = {"embeddings": [[0.1, 0.2]]}

    with pytest.raises(ValueError, match="returned 1 embeddings for 2 inputs"):
        embedder.embed_batch(["first", "second"], "add")
