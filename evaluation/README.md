...
| Convenience drivers | `run_abla_openai.sh`, `run_abla_qwen_full*.sh`, `score_openai.sh` | Arg-driven wrappers for the runs in the paper. |
| Table generators + data | `gen_table3_llm_calls.py`, `gen_token_table.py`, `make_results_tex.py`, [`paper_results/`](paper_results/) | Regenerate the paper's tables from vendored result summaries. |
...
For the open-weight runs (Qwen/Llama/DeepSeek) you instead need a running
[Ollama](https://ollama.com) server; point at it with `--ollama_base_url`.
...
**1. Install and key.** Install deps (top-level README: `pip install -r requirements.txt`)
and put your OpenAI key in a repo-root `.env`:
...
**5. Regenerate the paper's LaTeX tables.** The generators read the vendored
summaries in [`paper_results/`](paper_results/). To rebuild them from *your* fresh
runs, copy the four files over the vendored ones, then run the generators:
...
[`paper_results/README.md`](paper_results/README.md) for what each vendored file is,
...
> Ollama driver — `bash run_abla_qwen_full.sh sage qwen2.5:3b` then
> `bash run_abla_qwen_full_eval.sh sage qwen2.5:3b` (point `OLLAMA_HOST` at your server,
...
The three generators rebuild the paper's LaTeX tables from the **vendored result
