#!/bin/bash
# Run a single SAGE/mem0 evaluation on the OpenAI backend.
#
# Usage:
...
TECH="${1:?usage: run_openai.sh <mem0|sage> <gpt-4o-mini|gpt-4o|both> <smoke|20perc|full> [batch_size]}"
MODEL_ARG="${2:?usage: run_openai.sh <mem0|sage> <gpt-4o-mini|gpt-4o|both> <smoke|20perc|full> [batch_size]}"
SPLIT="${3:?usage: run_openai.sh <mem0|sage> <gpt-4o-mini|gpt-4o|both> <smoke|20perc|full> [batch_size]}"
BATCH_SIZE="${4:-2}"
