...
def _configure_mem0_local_runtime(args):
    ...
    # Default to disabling telemetry for local evaluation runs to avoid creating
...
def main():
    ...
    parser.add_argument(
        ...
        default="cloud",
        help="Mem0 backend for evaluation runs",
    )
    ...
    parser.add_argument(
        "--qdrant_path",
        type=str,
        default="/tmp/mem0_eval_qdrant_persist",
    ...
    parser.add_argument(
        "--history_db_path",
        type=str,
        default="/tmp/mem0_eval_history_persist.db",
        help="History DB path for mem0 ollama backend",
    )
    parser.add_argument(
        "--collection_name",
        type=str,
        default="mem0_eval_persist",
        help="Collection name for mem0 ollama backend",
    )
    parser.add_argument(
        "--action_stats_file",
        type=str,
        default="results/mem0_action_stats_eval.json",
        help="Path to write mem0 add action stats for evaluation runs",
    )
    ...
    try:
        if args.technique_type == "mem0":
            from src.memzero.add_sage import MemoryADDSAGE

            if args.method in ("add", "full"):
                add_started_at = time.perf_counter()
                memory_manager = MemoryADDSAGE(
