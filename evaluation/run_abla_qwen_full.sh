#!/bin/bash
# Run a single SAGE/mem0 ablation on the full LOCOMO dataset with an Ollama backend
# (representative open-weight Table-3 run; default model qwen2.5:3b).
#
# Usage:
#   run_abla_qwen_full.sh <tech> [ollama_model]
#     tech         : mem0 | sage
...
TECH="${1:?usage: run_abla_qwen_full.sh <mem0|sage> [ollama_model]}"
...
ACTION_STATS_FILE="$OUTPUT_FOLDER/${TECH}_${MODEL_SLUG}_vmf_action_stats_abla_full.json"

echo "Running ${TECH} ablation (${OLLAMA_MODEL}) on full locomo10..."
...
echo "Finished ${TECH} ablation (${OLLAMA_MODEL}) on full locomo10!"
